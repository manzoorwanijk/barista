...
const useMutationHandler = (): MH => {
	...
	const onUpdate = useCallback<MutationUpdater<Recurrence, RecurrenceCommonInput>>(
		({ proxy, entity: recurrence, input, mutationType }) => {
			...
			switch (mutationType) {
				case MutationType.Create:
					onCreateRecurrence({ proxy, recurrences, recurrence, datetimeIds });
					break;
				case MutationType.Update:
					onUpdateRecurrence({ proxy, recurrences, recurrence, datetimeIds });
					break;
				case MutationType.Delete:
					onDeleteRecurrence({ proxy, recurrences, recurrence });
					break;
			}
		},
