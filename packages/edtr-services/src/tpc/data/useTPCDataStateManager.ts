...
import { initialState, useTPCDataReducer } from './useTPCDataReducer';
import { useTPCInitialState } from './useTPCInitialState';
import type { TPCDataStateManager, TPCDataStateManagerHook } from './types';

type DSM = TPCDataStateManager;

export const useTPCDataStateManager: TPCDataStateManagerHook = (props) => {
	const initializer = useTPCInitialState(props);
	const dataReducer = useTPCDataReducer(initializer);
