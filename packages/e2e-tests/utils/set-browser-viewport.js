 ...
 */
export async function setBrowserViewport(viewport, page) {
	const dimensions = typeof viewport === 'string' ? PREDEFINED_DIMENSIONS[viewport] : viewport;

	await (page && page.setViewportSize(dimensions));
	await (page && waitForWindowDimensions(dimensions.width, dimensions.height, page));
}
