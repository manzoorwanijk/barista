...
const INSERTER_SEARCH_SELECTOR = '.block-editor-inserter__search-input,input.block-editor-inserter__search';
...
export async function toggleGlobalBlockInserter() {
	await page.click('.edit-post-header [aria-label="Add block"], .edit-site-header [aria-label="Add block"]');
}
...
async function waitForInserterCloseAndContentFocus() {
	await canvas().waitForFunction(() => document.activeElement.closest('.block-editor-block-list__layout') !== null);
}
...
export async function searchForPattern(searchTerm) {
	...
	const tab = await page.waitForXPath('//div[contains(@class, "block-editor-inserter__tabs")]//button[.="Patterns"]');
	await tab.click();
	await page.waitForSelector(INSERTER_SEARCH_SELECTOR);
	await page.focus(INSERTER_SEARCH_SELECTOR);
	await pressKeyWithModifier('primary', 'a');
...
export async function searchForReusableBlock(searchTerm) {
	...
	await page.waitForXPath('//div[contains(@class, "block-editor-inserter__tabs")]//button[text()="Reusable"]');
...
export async function insertBlock(searchTerm) {
	...
	const insertButton = await page.waitForXPath(`//button//span[contains(text(), '${searchTerm}')]`);
...
export async function insertPattern(searchTerm) {
	...
	const insertButton = await page.waitForXPath(`//div[@role = 'button']//div[contains(text(), '${searchTerm}')]`);
	await insertButton.click();
	// We should wait until the inserter closes and the focus moves to the content.
	await waitForInserterCloseAndContentFocus();
}
...
export async function insertReusableBlock(searchTerm) {
	...
	await page.waitForXPath('//*[@class="block-library-block__reusable-block-container"]');
}
...
export async function insertBlockDirectoryBlock(searchTerm) {
	...
	const insertButton = await page.waitForSelector('.block-directory-downloadable-blocks-list li:first-child button');
	await insertButton.click();
	// We should wait until the inserter closes and the focus moves to the content.
	await waitForInserterCloseAndContentFocus();
}
