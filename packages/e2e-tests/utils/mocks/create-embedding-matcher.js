...
function parameterEquals(parameterName, value) {
	return (request) => new URL(request.url()).searchParams.get(parameterName) === value;
}
...
export function createEmbeddingMatcher(url) {
	return (request) => isRESTRoute('/oembed/1.0/proxy')(request) && parameterEquals('url', url)(request);
}
