 ...
 */
export async function togglePreferencesOption(tabLabel, optionLabel, shouldBeChecked) {
	await clickOnMoreMenuItem('Preferences');
	const [tabHandle] = await page.$x(`//button[contains(text(), "${tabLabel}")]`);
	await tabHandle.click();
	const [optionHandle] = await page.$x(`//label[contains(text(), "${optionLabel}")]`);
	const isChecked = await page.evaluate((element) => element.control.checked, optionHandle);
