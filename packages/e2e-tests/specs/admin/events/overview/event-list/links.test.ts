...
import { EventsListSurfer, Goto, DateFormatter } from '@e2eUtils/admin';
...
describe(namespace, () => {
	const activeEventsTest = async (link: string) => {
		...
		const countBeforeUpdate = await eventsListSurfer.viewLinkAndCountEvents(link);
		...
		const startDate = await DateFormatter.eventStartDateFormat(dateToday);
		...
		const countfterUpdate = await eventsListSurfer.viewLinkAndCountEvents(link);

		// return count event event before and after for assertions
